let handler = async (m, { conn }) => {
    ...
    if (id in conn.misi) {
        conn.reply(m.chat, `sᴇʟᴇsᴀɪᴋᴀɴ ᴍɪsɪ ${conn.misi[id][0]} ᴛᴇʀʟᴇʙɪʜ ᴅᴀʜᴜʟᴜ`, m)
        throw false
    }
    if (new Date - user.lastmisi > 3600000) {
		...
		let hsl = `🕸 *ʜᴀsɪʟ ʙᴇʀʙᴜʀᴜ ${user.registered ? user.name : conn.getName(m.sender)}* 
${hewan1 ? `
...
`.trim()
		...
		setTimeout(() => {
			m.reply('ᴅᴀᴘᴀᴛ sᴀsᴀʀᴀɴ...')
		}, 14000)

		setTimeout(() => {
			m.reply('ᴍᴇɴᴄᴀʀɪ ᴍᴀɴɢsᴀ...')
		}, 0)
		user.lastmisi = new Date * 1
	} else m.reply(`sɪʟᴀʜᴋᴀɴ ᴍᴇɴᴜɴɢɢᴜ sᴇᴛᴇʟᴀʜ ${timers}, ᴜɴᴛᴜᴋ ᴍᴇɴʏᴇʟᴇsᴀɪᴋᴀɴ ᴍɪsɪ ᴋᴇᴍʙᴀʟɪ`)
}
